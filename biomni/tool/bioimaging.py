...
class SegmentationTool:
    ...
    def segment_with_nn_unet(self, image_path, output_dir, task_id, model_type="3d_fullres", 
                            folds=None, use_tta=False, num_threads=1, mixed_precision=True, 
                            verbose=True, auto_prepare_input=True, results_folder=None):
        """
...
class ImageRegistrationTool:
    ...
    def create_rigid_transform(self, 
                              fixed_image: sitk.Image, 
                              moving_image: sitk.Image,
                              initial_transform: Optional[sitk.Transform] = None) -> sitk.Transform:
        """
    ...
    def create_affine_transform(self, 
                               fixed_image: sitk.Image, 
                               moving_image: sitk.Image,
                               initial_transform: Optional[sitk.Transform] = None) -> sitk.Transform:
        """
    ...
    def create_deformable_transform(self, 
                                   fixed_image: sitk.Image, 
                                   moving_image: sitk.Image,
                                   number_of_control_points: int = 4) -> sitk.Transform:
        """
    ...
    def setup_registration_method(self, 
                                 ...
                                 gradient_convergence_tolerance: float = 1e-6) -> sitk.ImageRegistrationMethod:
        """
        ...
        if optimizer == "gradient_descent":
            ...
            registration_method.SetOptimizerAsAmoeba(
                numberOfIterations=number_of_iterations,
                parametersConvergenceTolerance=gradient_convergence_tolerance
            )
        else:
    ...
    def register_images(self, 
                       ...
                       moving_image: sitk.Image,
                       transform: sitk.Transform,
                       registration_method: sitk.ImageRegistrationMethod) -> Tuple[sitk.Transform, sitk.Image]:
        """
        ...
        def command_iteration():
            logger.info(f"Iteration: {registration_method.GetOptimizerIteration()}, "
...
def segment_with_nn_unet(image_path, output_dir, task_id, model_type="3d_fullres", 
                        folds=None, use_tta=False, num_threads=1, mixed_precision=True, 
                        verbose=True, auto_prepare_input=True, results_folder=None):
    """Convenience function for nnUNet segmentation"""
    tool = SegmentationTool()
    return tool.segment_with_nn_unet(image_path, output_dir, task_id, model_type, folds, 
                                   use_tta, num_threads, mixed_precision, verbose, 
                                   auto_prepare_input, results_folder)
...
def quick_rigid_registration(fixed_image_path: str, 
                            ...
                            learning_rate: float = 0.01,
                            number_of_iterations: int = 100,
                            gradient_convergence_tolerance: float = 1e-6) -> Dict:
    """
    ...
    registration_method = tool.setup_registration_method(
        transform, metric, optimizer, learning_rate, 
        number_of_iterations, gradient_convergence_tolerance
    )
    
    # Perform registration
    final_transform, registered_image = tool.register_images(
        fixed_image, moving_image, transform, registration_method
    )
...
def quick_affine_registration(fixed_image_path: str, 
                             moving_image_path: str, 
                             output_dir: str,
                             metric: str = "mutual_information",
                             optimizer: str = "gradient_descent",
...
def quick_deformable_registration(fixed_image_path: str, 
                                 moving_image_path: str, 
                                 output_dir: str,
                                 metric: str = "mutual_information",
                                 optimizer: str = "gradient_descent",
                                 ...
                                 number_of_control_points: int = 4) -> Dict:
    """
...
def batch_register_images(fixed_image_path: str, 
        ...
        try:
            if transform_type == "rigid":
                result = quick_rigid_registration(
                    fixed_image_path, moving_image_path, individual_output_dir,
                    metric, optimizer, preprocess, create_visualizations,
                    learning_rate, number_of_iterations, gradient_convergence_tolerance
                )
